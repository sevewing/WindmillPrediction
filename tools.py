...
linear_law = lambda x1, x2, y1, y2, z: ((y2 - y1) / (x2 - x1)) * z
udic = {}
...
def _pow_interp(v1, v2, z1, z2, z_hat):
    a1 = abs(v1)
    a2 = abs(v2)
    ...
    return round(pow_law(v1, z1, z_hat, a),3)


def wind_interp(u1, v1, u2, v2, z_hat):
    u_i = round(_pow_interp(u1, u2, 10, 100, z_hat),3)
    v_i = round(_pow_interp(v1, v2, 10, 100, z_hat),3)
    return u_i, v_i

def wind_interp_pow(u, v, z_hat, rl):
